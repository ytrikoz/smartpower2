...
function onWatthChange(item) {
	if (item.last < 10) {
		$('#watth_ss').sevenSeg({ digits: 4, decimalPlaces: 3 });
	} else if (item.last < 100) {
		$('#watth_ss').sevenSeg({ digits: 4, decimalPlaces: 2 });
	} else if (item.last < 1000) {
		$('#watth_ss').sevenSeg({ digits: 4, decimalPlaces: 1 });
	} else {
		$('#watth_ss').sevenSeg({ digits: 4, decimalPlaces: 0 });
	}
...
const template = {
	...
	html() {
		...
		for (key in this) {
			if (Object.prototype.hasOwnProperty.call(this, key)) { break; }
		}
...
function enableLivePVI(enabled = true) {
...
function sendNetwork(wifi, ssid, passwd, dhcp, ipaddr, netmask, gateway, dns) {
	send(`${SET_NETWORK}${wifi},${ssid},${passwd},${dhcp ? '1' : '0'},${ipaddr},${netmask},${gateway},${dns}`);
}
...
function onload() {
	...
	try {
		...
		ws.onmessage = function onWSMessage(msg) {
			enableUI();
			parseMessage(msg.data);
		};
		ws.onopen = function onWSOpen() {
			const page = getPageIndex($('.ui-page-active').jqmData('title'));
			if (page > 0) {
				send(`${GET_PAGE}${[page]}`);
			}
		};
		ws.onclose = function onWSClose() {
			enableUI(false);
...
$(document).ready(() => {
	...
	$('#btn_save_network').click(() => {
		...
		const ipaddr = $('#tf_ipaddr').val();
		const netmask = $('#tf_netmask').val();
		const gateway = $('#tf_gateway').val();
		const dns = $('#tf_dns').val();

		sendNetwork(wifi, ssid, passwd, dhcp, ipaddr, netmask, gateway, dns);
	...
	$('#dlg_restart_yes').click(() => {
		$('.ui-pagecontainer').pagecontainer('change', '/restart=5', {});
	});
