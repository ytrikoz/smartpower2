...
class Measurement {
	...
	setValue(newValue) {
		...
		let changes = false;
		// ToDo: low cut filter/ignoring values fluctuation at start
	...
	get stat() {
		// ToDo calc some useful stat data for meassure, use treegrid
		return `${this.name} - ${this.counter} samples from ${this.min} to ${this.max}`;
	}
