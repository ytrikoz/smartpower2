...
/*-----------------------------------------------------------------------------------------------------------------
 ...
 input: string buffer: the content of the buffer will be displayed and you can
edit it lineLength   : maximum number of characters the line can have and can be
edited
...
void editLine(char *str, uint8_t lineLength) {
    ...
    while ((ch = getch()) != KEY_CR) {
        ...
        default:
            if (curlen < lineLength && (ch & 0x7F) >= 32 &&
                (ch & 0x7F) <
