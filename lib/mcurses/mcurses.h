 ...
 * V1.1 2017 01 13 ChrisMicro, addepted as Arduino library, MCU specific
 *functions removed
 *
...
void move(uint_fast8_t,
          uint_fast8_t);      // move cursor to line, column (home = 0, 0)
...
void getnstr(
    char *str,
    uint_fast8_t maxlen);  // read a string (with mini editor functionality)
...
void curs_set(
    uint_fast8_t);   // set cursor to: 0=invisible 1=normal 2=very visible
...
#define mvaddch(y, x, c) \
    move((y), (x)), addch((c))  // move cursor, then add character
#define mvaddstr(y, x, s) \
    move((y), (x)), addstr((s))  // move cursor, then add string
#define mvaddstr_P(y, x, s) \
    move((y), (x)), addstr_P((s))  // move cursor, then add string (PROGMEM)

#define mvinsch(y, x, c) \
    move((y), (x)), insch((c))  // move cursor, then insert character
#define mvdelch(y, x) \
    move((y), (x)), delch()  // move cursor, then delete character
#define mvgetnstr(y, x, s, n) \
    move((y), (x)), getnstr(s, n)  // move cursor, then get string
#define getyx(y, x) \
    y = mcurses_cury, x = mcurses_curx  // get cursor coordinates
