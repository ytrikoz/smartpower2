...
bool ConfigHelper::setNetworkSTAConfig(uint8_t wifi, const char *ssid, const char *passwd, bool dhcp, const char *ipaddr, const char *netmask, const char *gateway, const char *dns)
{
    bool changes = setWiFiMode(wifi) | setSSID(ssid) | setPassword(passwd) | setDHCP(dhcp) | setIPAddress(ipaddr) | setNetmask(netmask) | setGateway(gateway) | setDNS(dns);
    return changes;
}
...
const char *ConfigHelper::getSSID()
{
    return getStrValue(SSID);
}

const char *ConfigHelper::getSSID_AP()
{
    return getStrValue(AP_SSID);
}

const char *ConfigHelper::getPassword()
{
    return getStrValue(PASSWORD);
}

const char *ConfigHelper::getPassword_AP()
{
    return getStrValue(AP_PASSWORD);
}

IPAddress ConfigHelper::getIPAddr_AP()
{
    return getIPAddrValue(AP_IPADDR);    
}

bool ConfigHelper::getDHCP()
{
    return getBoolValue(DHCP);
}

IPAddress ConfigHelper::getDNS()
{
    return getIPAddrValue(DNS);
}

IPAddress ConfigHelper::getNetmask()
{
    return getIPAddrValue(NETMASK);
}
...
{
    return getIPAddrValue(IPADDR);
}

const char *ConfigHelper::getIPStr()
{
    return getStrValue(IPADDR);
}

const char *ConfigHelper::getNetmaskStr()
{
    return getStrValue(NETMASK);
}

const char *ConfigHelper::getGatewayStr()
{
    return getStrValue(GATEWAY);
}
const char *ConfigHelper::getDNSStr()
{
    return getStrValue(DNS);
}

float ConfigHelper::getOutputVoltage()
{
    return getFloatValue(OUTPUT_VOLTAGE); 
}
