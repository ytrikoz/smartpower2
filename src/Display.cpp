...
const char* Display::getParam(uint8_t row)
{
    return line[row].param;

}

void Display::setParamLine(uint8_t row, const char *paramStr, const char *valueStr)
{

    if (
        setstr(this->line[row].param, paramStr, LCD_COLS + 1) |
...
void Display::update(boolean forced)
{
    ...
    if (!forced &&
        ((millis() - this->lastUpdateTime) < REFRESH_INTERVAL) &&
        (this->lastUpdateTime != 0))
    {
        return;
    }
...
void Display::show(const String &s)
{
    this->lcd->print(s);
}
