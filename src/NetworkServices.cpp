...
void NetworkServices::setOutput(Stream *stream)
{
	output = stream;
}
...
bool NetworkServices::begin_dns(const char *domain, IPAddress ip, uint16_t port)
{
	dns = new DNSServer();
	return dns->start(port, domain, ip);
}

bool NetworkServices::begin_mdns(const char *hostname, uint16_t telnet_port, uint16_t http_port, uint16_t ota_port)
{
