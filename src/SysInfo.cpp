...
String getHostIPInfo()
{
    ...
    switch(status)
    {
        case STATION_IDLE: str += F("idle");
        case STATION_CONNECTING: str += F("connecting");
        case STATION_WRONG_PASSWORD: str += F("wrong password");
        case STATION_NO_AP_FOUND: str += F("no ap found");
        case STATION_CONNECT_FAIL: str += F("connect fail");
        case STATION_GOT_IP: 
        {
            str += F("got ip ");
...
String getHeapStatistic()
{
	...
	char buf[64];
	sprintf(buf, "free %s max %s frag %d%%", formatSize(free).c_str(), formatSize(max).c_str(), frag);
	return String(buf);	
}
