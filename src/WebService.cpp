...
void WebService::setOutputPrint(Print *output)
{
    this->output = output;
}

void WebService::onNoContent()
{
...
void WebService::socketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t lenght)
{    
...
String WebService::getFilePath(const String uri)
{
    return String(_root) + uri;
}
...
void WebService::initSSDP(uint16_t port)
{
	...
	ssdp->setModelURL(F("https://wiki.odroid.com/accessory/power_supply_battery/smartpower2"));
